	...
	public void render(Graphics g) {
		...
		if (deltaX == 0 && deltaY == 0) {
			...
			if ((int) player.getY() < (int) y) {
				...
				rotate(g, 1).drawImage(move_up.getCurrentImage(Texture.boss_up[ID]), (int) 0, (int) 0, width, height, null);
			} else {
				// System.out.println("down");
				rotate(g, -1).drawImage(move_down.getCurrentImage(Texture.boss_down[ID]), (int) 0, (int) 0, width, height,
						null);
			}
		}

		if (deltaX < 0) {
			g.drawImage(move_left.getCurrentImage(Texture.boss_left[ID]), (int) (x - width / 2), (int) (y - height / 2),
					width, height, null);
		}
		if (deltaX > 0) {

			g.drawImage(move_right.getCurrentImage(Texture.boss_right[ID]), (int) (x - width / 2), (int) (y - height / 2),
					width, height, null);

		}
		if (deltaY < 0) {
			g.drawImage(move_up.getCurrentImage(Texture.boss_up[ID]), (int) (x - width / 2), (int) (y - height / 2), width,
					height, null);

		}
		if (deltaY > 0) {
			g.drawImage(move_down.getCurrentImage(Texture.boss_down[ID]), (int) (x - width / 2), (int) (y - height / 2),
					width, height, null);
